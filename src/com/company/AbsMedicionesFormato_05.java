...
public class AbsMedicionesFormato_05 extends AbsMediciones {
    public JFrame framePrincipal;// define objeto frame padre que viene en el constructor
    ...
    public AbsMedicionesFormato_05(String tipodeFormatoaLeer, JFrame framePrincipal) {
    ...
    public  void PrincipalDevMediciones() throws IOException, ParseException { // Aplicacion principal para el chequeo de las mediciones
        ...
        String palabraFormato2L1="Equipo";
        String palabraFormato3L1="";
        String palabraFormato4L1="";
        String palabraFormato5L1="FECHA|HORA|KWH|KVARH|ESTADO";
        ...
        int longitudDigitosNombreArchivo=20; // VALOR QUE PERMITE CHEQUEAR EL CUMPLIMIENTO DE LONGITUD DE CARACTERES DEL ARCHIVO
        //==============================================================


        AbsArchivoFormato_05 absArchivo=new AbsArchivoFormato_05(palabraFormato5L1,0,getDirectorio(),tipodeFormatoaLeer,longitudDigitosNombreArchivo) ;// Define el formato a ser leido -->Instancia la clase Archivo e inicializa numerolinea con cero>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>><

        System.out.println(" Formato_05 ------------------------>>>> el directorio definido que va a a clase Achivo es "+directorio);
        System.out.println("la palabra de referenca en la ClsArch.. es: "+absArchivo.getPrimeraPalabraValidaArchivo());
        ...
        armaENcabezadoArchReporte0405(); // Arma encabezados en los archivos: ReporteDetalle.txt y Reporte.txt
       ...
       //-----  control frame padre
        ...
        if(varHabilitaCompilador==1) {
            ...
            absArchivo.principalCompilacionArchivosFormato0405(directorio);
        }
        ...
        //<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
        // implementacion metodo de COMPILACION

        //*** Inicio compilacion de archivos
...
//***********************************************
                ...
                public void mouseEntered(MouseEvent mouseEvent) {
                    ...
                    if(resp==JOptionPane.YES_OPTION){
                        try {
                            ...
                            System.out.println("Se produjo un error al aperturar los archivos de Resultado!!!");
                        }
    ...
    public void escribeResultadosenBufferReporte(AbsArchivoFormato_05 absArchivo) throws IOException {
    ...
    public void lazoprincipal2(AbsArchivoFormato_05 absArchivo, String nombreArchivo,int numlineas) throws IOException, ParseException {
        ...
        for (int l=0;l<xlistaArchivos.size();l++) {
            ...
            // ajusta valor de inicializacion con valor real deL nro de lineas
             ...
             // ------------------------------------------------------------------
            // Variable para el manejo de errores, parametro devuelto por cada metodo "GenerarToken...."
                String TokenErrorFormat="";
            //--------------------------------------------------
            ...
            if ((setup.CnroAplicacionEspecial == 0) && (tipodeFormatoaLeer.equals("FORMATO_05"))) { // no hay caso especial y se habilita el metodo segun FORMATO XX
                System.out.println("<< ESTA ES LA DERIVACION FORMATO 0 ------------------");
                TokenErrorFormat=absArchivo.generarTokensFormato_05(path, nombreArchivo, numlineas);
               // JOptionPane.showMessageDialog(null,"valor "+TokenErrorFormat);
            }
            ...
            try {
                String calificacionArchivoMedicion=absArchivo.calificarMedicion_Formato05_AES_MG(nombreArchivo,   //   !!!!!!!!  LLama al metodo de calificacion de
                ...
                if ((calificacionArchivoMedicion.equals("")||calificacionArchivoMedicion.equals("A")) && TokenErrorFormat=="" ){
                    tempMensajeCalificacion="Normal";
                }else{
                    if (calificacionArchivoMedicion.equals("")){
                        tempMensajeCalificacion = "Normal con " + TokenErrorFormat;

                    }else {
                        tempMensajeCalificacion = calificacionArchivoMedicion + " | " + TokenErrorFormat;// se asigna el valor que viene de la calificacion
                    }
                }
                //-------------------------------------------------------------------------------------
                // Escribe el error recibido del metodo GenerarToken mediante TokenErrorFormat
                if(TokenErrorFormat!="") {
                    printErrorArchivoReport.append(sbCrLf()+TokenErrorFormat + "  en archivo |" + absArchivo.extraeNombreArchivoBaseSinrutaNiextension(nombreArchivo) + sbCrLf());
                }
                //------------------------------------------------------------------------
            ...
            double energiaMarcadaAnomala=absArchivo.sumadeRegistrosMarcadoAnomalo(absArchivo.archEnergia);
           //absArchivo.numMuestraAnormal= cantidad de muestras anormales
            ...
            double redondeaEnergiaPromedio=absArchivo.redondeaDouble2decimales(absArchivo.calcularPromedio((absArchivo.archEnergia)));
                    redondeaEnergiaPromedio=absArchivo.redondeaDouble2decimales((absArchivo.archTotalEnergia-energiaMarcadaAnomala)/(absArchivo.archEnergia.size()-absArchivo.numMuestraAnormal));
            double redondeaEnergiaMin=absArchivo.redondeaDouble2decimales(absArchivo.calcularMinimoSinRegAnomalos(absArchivo.archEnergia));
            double redondeaEnergiaMax=absArchivo.redondeaDouble2decimales(absArchivo.calcularMaximoSinAnomalos(absArchivo.archEnergia));
            double redondeaFP=absArchivo.redondeaDouble2decimales(absArchivo.calcularPromedio(absArchivo.archFP));
            System.out.println(absArchivo.archTotalEnergiaReactiva + " round total reactivo " +redondeaEnergiaTotalReactiva);
            ...
            // Factor de Carga
            //Integer tempFactorCarga= Math.toIntExact(Math.round(redondeaEnergiaPromedio / redondeaEnergiaMax));
                Double tempFactorCarga=0.0;
            if (redondeaEnergiaMax!=0) {// manejo de error en valor de energia maxima
                tempFactorCarga = absArchivo.redondeaDouble2decimales(redondeaEnergiaPromedio / redondeaEnergiaMax);
            }

            //----------CONVERSION DE FORMATO DATE mm/DD/yyyy PARA llamar a la rutina que calcula los dias con formato entrada dd/mm/yyyy-------------------
            String fechaInicial=absArchivo.enroqueFecha_diaMes(absArchivo.archFecha.get(0).toString());
            String fechaFinal=absArchivo.enroqueFecha_diaMes(absArchivo.archFecha.get((absArchivo.archFecha.size() - 1)).toString());
           // Valida que presentar llamando al metodo
            String tempdias="";
            if (flagPresentardiasenResumen(calificacionArchivoMedicion)){// chequea Si contiene "D" no calcula dias
                tempdias= String.valueOf(absArchivo.calcularDiasString(fechaFinal,fechaInicial));
            }else{
                tempdias="--";
            }
            ...
            listaReporteDetalle.add(" Energía total registrada:      " + tab()+ redondeaEnergia + " "+ absArchivo.multiploUnidadPotencia+"h"+ sbCrLf());
            ...
            listaReporteDetalle.add(" Energía reactiva total:        " + tab()+ redondeaEnergiaTotalReactiva + " "+ absArchivo.getMultiploUnidadPotenciaReactiva()+"h"+sbCrLf());
            listaReporteDetalle.add(" Factor de carga:           " + tab()+tab()+ tempFactorCarga+sbCrLf()); // tomado de promediar valores ver linea 205
            ...
            listaReporteResumen.add(parteDelNombre[parteDelNombre.length - 1].toUpperCase() +spaceAscii(nroEspaciosBlanco)+tab()+  "  "+
            ...
            if(varHabilitaCompilador==1 && suicheCalificacionCompilable==1){
                ...
                if(flagFaltaEncabezadoArchivo) {//imprime el encabezado solo para el primer archivo
                    ...
                    printMedIntegradas.write("Fechas" + "|" + "Hora" + "|" + "Voltaje" + "|" + "Potencia" + "|" + "PotenciaReactiva"+ "|" + "FP" + "|" + "Anomalia" + "|" +
                            "IdUsuario" + "|" + "Tarifa-Estrato" + "|" + "NombreArchivo" + "|" +"UPR"+ "|" + "MultiploUnidadPotencia" + sbCrLf());
                }
                //------------------------------------------------------------
                ...
                for (int i = 0; i < absArchivo.archHora.size(); i++) {// inicializa el lazo para barrer totos los registros leidos
                    ...
                    printMedIntegradas.write(absArchivo.archFecha.get(i) + "|" + absArchivo.archHora.get(i) + "|" +
                            absArchivo.archVoltaje.get(i) + "|" + funcModuloArchPotencia + "|" +
                            absArchivo.archPotenciaReactiva.get(i) + "|"+ absArchivo.archFP.get(i) + "|" +
                            absArchivo.archflagAnormalidad.get(i) + "|"+
                            absArchivo.buscarUsuarioxNombArchivoInstalacion(absArchivo.extraeNombreArchivoBaseSinrutaNiextension(nombreArchivo))+
                            "|"+absArchivo.buscarTarifaUsuarioxNombArchivoInstalacion(absArchivo.extraeNombreArchivoBaseSinrutaNiextension(nombreArchivo))+
                            "|"+ absArchivo.extraeNombreArchivoBaseSinrutaNiextension(nombreArchivo)+
                            "|"+ absArchivo.buscarUsuarioUPR(absArchivo.extraeNombreArchivoBaseSinrutaNiextension(nombreArchivo))+
        ...
        //printErrorArchivoReport.write(sbCrLf()+"Archivos procesados: "+xlistaArchivos.size()+ " | Compilados: "+nroArchivosCompilados+ " | Porcentaje: "+(Math.round(nroArchivosCompilados/xlistaArchivos.size()*100))+" % | "+sbCrLf()+sbCrLf());
    }//Fin lazoprincipal2


    //---------- metodo de conversion de formato fecha de dd/mm/YYYY a mm/dd/AAAA
    public String convertirDateformatoMMddAAAAaddMMAAA(String fecha){
