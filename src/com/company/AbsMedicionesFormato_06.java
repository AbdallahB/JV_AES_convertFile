...
public class AbsMedicionesFormato_06 extends AbsMediciones{
    public JFrame framePrincipal;// instancia tipo de objeto del frame padre
    ...
    public AbsMedicionesFormato_06(String tipodeFormatoaLeer, JFrame framePrincipal) {
        super(tipodeFormatoaLeer);
        this.framePrincipal = framePrincipal;
    }

    // Clase principal que se instancia como aplicacion principal Baes Mediciones
    ...
    public  void PrincipalDevMediciones() throws IOException, ParseException { // Aplicacion principal para el chequeo de las mediciones
        ...
        String palabraFormato6L1="meterId";//meterId
        ...
        AbsArchivoFormato_06 absArchivo=new AbsArchivoFormato_06(palabraFormato6L1,0,getDirectorio(),tipodeFormatoaLeer,longitudDigitosNombreArchivo) ;// Define el formato a ser leido -->Instancia la clase Archivo e inicializa numerolinea con cero>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>><

        System.out.println(" Formato_06 ------------------------>>>> el directorio definido que va a a clase Achivo es "+directorio);
        System.out.println(sbCrLf()+"La palabra de referenca en la ClsArch.. es: "+absArchivo.getPrimeraPalabraValidaArchivo());
    ...
    public void escribeResultadosenBufferReporte(AbsArchivoFormato_06 absArchivo) throws IOException {
    ...
    public void lazoprincipal2(AbsArchivoFormato_06 absArchivo, String nombreArchivo, int numlineas)
        ...
        for (int l=0;l<xlistaArchivos.size();l++) {
            ...
            if ((setup.CnroAplicacionEspecial == 0) && (tipodeFormatoaLeer.equals("FORMATO_06"))) { // no hay caso especial y se habilita el metodo segun FORMATO XX
                System.out.println("<< ESTA ES LA DERIVACION FORMATO 06 ------------------");
                absArchivo.generarTokensFormato_06(path,nombreArchivo,numlineas);
            }
...
// TRATAMIENTO DE CASOS ESPECIALES  en Setup CnroAplicacionEspecial=1 que era san juan
            ...
            if(varHabilitaCompilador==1 && suicheCalificacionCompilable==1){
                ...
                if(cuentalosxListaArchivosProcesados==1) {// imprime el encabezado solo para el primer archivo
                ...
                for (int i = 0; i < absArchivo.archHora.size(); i++) {// inicializa el lazo para barrer totos los registros leidos
                    ...
                    // -----> BLOQUE PRUEBA DE CORRECCION DE TRANSFORMACION
                    // Utiliza el campo nro 3 de Usuarios seleccionados para cargar el factor correccion, y de ahi
                    // lo carga en la variable FactorTransformacionxNombArchivoInstalacion
                   //    double potencia= (double) absArchivo.archPotencia.get(i);
                   // String StrFactor= absArchivo.buscarFactorTransformacionxNombArchivoInstalacion(absArchivo.extraeNombreArchivoBaseSinrutaNiextension(nombreArchivo));
                   // double factorK= Double.parseDouble(StrFactor);
                    //JOptionPane.showMessageDialog(null, factorK +" valor sin factor= "+ potencia + " | potencia con factor= "+potencia*factorK);
                    //==================================
