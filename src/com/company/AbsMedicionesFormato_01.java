...
public class AbsMedicionesFormato_01 extends AbsMediciones{
    ...
    public AbsMedicionesFormato_01(String tipodeFormatoaLeer, JFrame framePrincipal) {
        super(tipodeFormatoaLeer);
        this.framePrincipal = framePrincipal;
    }
//-----------------------------------------------------------------------------------------
    public AbsMedicionesFormato_01(String leyendoFormatox) {
        super(leyendoFormatox);
    }
    ...
    public  void PrincipalDevMediciones() throws IOException, ParseException { // Aplicacion principal para el chequeo de las mediciones
        ...
        String palabraFormato1L1="\"Data\";\"Time\";\"Period(s)\";\"Unit";
        ...
        AbsArchivoFormato_01 absArchivo=new AbsArchivoFormato_01(palabraFormato1L1,0,getDirectorio(),tipodeFormatoaLeer,longitudDigitosNombreArchivo) ;// Define el formato a ser leido -->Instancia la clase Archivo e inicializa numerolinea con cero>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>><

       // System.out.println(" Formato_01 ------------------------>>>> el directorio definido que va a a clase Achivo es "+directorio);
        ...
        //         System.out.println(i+ "  Archivo a procesar "+xlistaArchivos.get(i));
       // }
        ...
        if(varHabilitaCompilador==1) {
            ...
            absArchivo.principalCompilacionArchivos(directorio);// Muestra dialogo
                                                                // para seleccionar ARchivo de Usuarios
        }
    ...
    public void escribeResultadosenBufferReporte(AbsArchivoFormato_01 absArchivo) throws IOException {
    ...
    public void lazoprincipal2(AbsArchivoFormato_01 absArchivo, String nombreArchivo, int numlineas) throws IOException, ParseException {
        ...
        for (int l=0;l<xlistaArchivos.size();l++) {
            ...
            if ((setup.CnroAplicacionEspecial == 0) && (tipodeFormatoaLeer.equals("FORMATO_01"))) { // no hay caso especial y se habilita el metodo segun FORMATO XX
                System.out.println("<< ESTA ES LA DERIVACION FORMATO 01 ------------------"+ tipodeFormatoaLeer);
                absArchivo.generarTokensFormato_01(path,nombreArchivo,numlineas);
            }
            ...
            if ((setup.CnroAplicacionEspecial == 0) && (tipodeFormatoaLeer.equals("FORMATO_08"))) { // no hay caso especial y se habilita el metodo segun FORMATO XX
                ...
                //absArchivo.generarTokensFormato_08(path,nombreArchivo,numlineas);
...
// TRATAMIENTO DE CASOS ESPECIALES  en Setup CnroAplicacionEspecial=1 que era san juan
            ...
            try {
                ...
                listaReporteDetalle.add(" NOMBRE DEL ARCHIVOx:      " +tab()+ parteDelNombre[parteDelNombre.length - 1].toUpperCase() + sbCrLf());
            ...
            System.out.println("Escribe datos del archivo procesado encabezados LazoPrincipal2");
            String calificacionArchivoMedicion=absArchivo.calificarMedicionM(nombreArchivo,   //   !!!!!!!!  LLama al metodo de calificacion de
                    setup.CinicialesNombEmpresas,
                    setup.CdiasPermanencia, setup.CdiasMinValidez,
                    setup.CDigitoAnoValido, setup.ChabilitacionChequeo, setup.Ccadencia,setup.getCestratosList(),setup.getCnombTarifas());
            ...
            for (int calif=0;calif<setup.calificacionesPermitidas.size();calif++){ // Barre todas las claificaciones permitidas en SETUP
                if (setup.calificacionesPermitidas.get(calif).equals(calificacionArchivoMedicion)){
                    ...
                    System.out.println(" x1 el control de calificación no habilita la compilacion|"+
                            setup.calificacionesPermitidas.get(calif)+"|"+calificacionArchivoMedicion);
                }
            }
            ...
            if(varHabilitaCompilador==1 && suicheCalificacionCompilable==1){
                System.out.println("HABILITADA COMPILACION DE ARCHIVOS >>>"+varHabilitaCompilador+suicheCalificacionCompilable);
