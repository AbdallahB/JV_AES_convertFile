...
public class AbsMedicionesFormato_02 extends AbsMediciones{
    ...
    public AbsMedicionesFormato_02(String tipodeFormatoaLeer, JFrame framePrincipal) {
    ...
    public  void PrincipalDevMediciones() throws IOException, ParseException { // Aplicacion principal para el chequeo de las mediciones
        ...
        AbsArchivoFormato_02 absArchivo=new AbsArchivoFormato_02(palabraFormato2L1,0,getDirectorio(),tipodeFormatoaLeer,longitudDigitosNombreArchivo) ;// Define el formato a ser leido -->Instancia la clase Archivo e inicializa numerolinea con cero>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>><

        System.out.println(" Formato_02 ------------------------>>>> el directorio definido que va a a clase Achivo es "+directorio);
...
//***********************************************
    ...
    public void escribeResultadosenBufferReporte(AbsArchivoFormato_02 absArchivo) throws IOException {
    ...
    public void lazoprincipal2(AbsArchivoFormato_02 absArchivo, String nombreArchivo,int numlineas) throws IOException, ParseException {
        ...
        for (int l=0;l<xlistaArchivos.size();l++) {
            ...
            if ((setup.CnroAplicacionEspecial == 0) && (tipodeFormatoaLeer.equals("FORMATO_02"))) { // no hay caso especial y se habilita el metodo segun FORMATO XX
                System.out.println("<< ESTA ES LA DERIVACION FORMATO 02 ------------------");
                if ((absArchivo.tipoArchivo == "M")) {             // ******* Define procesamiento de los archivos tipo MONOFASICO
                    try{
                        absArchivo.generarTokensFormato_02Monofasico(path, nombreArchivo, numlineas); //**** llama PROCESAR EL ARCHIVO MONOFASICO CON LAS MUESTRAS
                    } catch (IOException e) {
                        e.printStackTrace();
                    ...
                    absArchivo.generarTokensFormato_02Trifasico(path, nombreArchivo, numlineas);
                }
            }
