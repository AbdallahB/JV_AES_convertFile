...
public class AbsMedicionesFormato_09 extends AbsMediciones {
    public JFrame framePrincipal;

    public AbsMedicionesFormato_09(String tipodeFormatoaLeer) {
    ...
    public AbsMedicionesFormato_09(String tipodeFormatoaLeer, JFrame framePrincipal) {
        super(tipodeFormatoaLeer);
        this.framePrincipal = framePrincipal;
    }
    ...
    public  void PrincipalDevMediciones() throws IOException, ParseException { // Aplicacion principal para el chequeo de las mediciones
        ...
        String palabraFormato2L1="Equipo";//"C:\\a\\a";//Caso EDEA
        String palabraFormato3L1="";
        String palabraFormato4L1="";
        String palabraFormato5L1="";
        String palabraFormato6L1="";
        String palabraFormato7L1="Equipo";//meterId
        String palabraFormato8L1="Equipo";//"C:\\a\\a";//Caso EDEA
        String palabraFormato9L1="meterNo,intervalStartDate,intervalValue,dataDefinitionId,intervalStatusId";//<- valido AMI V3.0.6, en
                                                                                                        // V3.0.5 = "Equipo";Caso EDEA
        ...
        int longitudDigitosNombreArchivo=9;////// CAMBIADO DE 8 A 9 PARA PROCESAR EDEA SEPTIEMBRE 2021 XXXXXXX
        AbsArchivoFormato_09 absArchivo=new AbsArchivoFormato_09(palabraFormato9L1,0,getDirectorio(),tipodeFormatoaLeer,longitudDigitosNombreArchivo) ;// Define el formato a ser leido -->Instancia la clase Archivo e inicializa numerolinea con cero>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>><

        System.out.println(" Formato_09AMI ------------------------>>>> el directorio definido que va a a clase Achivo es "+directorio);
     ...
     // Deshabilitada l lectura del archivo de Usuarios Sleccionados para AMI
            System.out.println("Deshabilitada la lectura del archivo Usuarios para AMI ");
    //zzz        absArchivo.principalCompilacionArchivos(directorio);
        }
        ...
        //************************************************************************************************************               ****   *****
        lazoprincipal2(absArchivo,nombreArchivo,numlineas); // Rutina complemento del lazo principal para barrer
        // la xlista con todos los archivos
...
//***********************************************
    ...
    public void escribeResultadosenBufferReporte(AbsArchivoFormato_09 absArchivo) throws IOException {
    ...
    public void lazoprincipal2(AbsArchivoFormato_09 absArchivo, String nombreArchivo,int numlineas) throws IOException, ParseException {
        boolean flagFaltaEncabezadoArchivo=true;
        System.out.println("lazo AMI Ppal2");
        ...
        for (int l=0;l<xlistaArchivos.size();l++) {
            ...
            if ((setup.CnroAplicacionEspecial == 0) && (tipodeFormatoaLeer.equals("FORMATO_09"))) { // no hay caso especial y se habilita el metodo segun FORMATO XX
                System.out.println("<< ESTA ES LA DERIVACION FORMATO 09 AMI ------------------");
                if ((absArchivo.tipoArchivo == "M")) {             // ******* Define procesamiento de los archivos tipo MONOFASICO
                    ...
                    absArchivo.generarTokensFormato_09AMI(path, nombreArchivo, numlineas);
                }
            }
            if ((setup.CnroAplicacionEspecial == 0) && (tipodeFormatoaLeer.equals("FORMATO_09"))) { // no hay caso especial y se habilita el metodo segun FORMATO XX
...
// TRATAMIENTO DE CASOS ESPECIALES  en Setup CnroAplicacionEspecial=1 que era san juan
            ...
            try {
                ...
                System.out.println("xCALIFICACION "+tempMensajeCalificacion);
      ...
      //      double redondeaFP=absArchivo.redondeaDouble2decimales(absArchivo.calcularPromedio(absArchivo.archFP));
            ...
            System.out.println("xFin redondeos ");
            ...
            listaReporteResumen.add(parteDelNombre[parteDelNombre.length - 1].toUpperCase() +spaceAscii(nroEspaciosBlanco+4)+ tab()+
     ...
     // XXX PARA AMI formato 09
            suicheCalificacionCompilable=1;
     // if ((absArchivo.buscarUsuarioxNombArchivoInstalacion(absArchivo.extraeNombreArchivoBaseSinrutaNiextension(nombreArchivo))=="NoEncontrado")&&
     //         (varHabilitaCompilador==1)){
     ...
     // }
            //-----------------------------------------
            //-----------------------------------------
            System.out.println(absArchivo.extraeNombreArchivoBaseSinrutaNiextension(nombreArchivo)+" multiploUnidad "+absArchivo.multiploUnidadPotencia);
            if(varHabilitaCompilador==1 && suicheCalificacionCompilable==1 &&
                    (absArchivo.multiploUnidadPotencia.toUpperCase().equals("KW")||absArchivo.multiploUnidadPotencia.toUpperCase().equals("W"))){
                System.out.println("HABILITADA COMPILACION DE ARCHIVOS");
                ...
                if(flagFaltaEncabezadoArchivo) {//imprime el encabezado solo para el primer archivo
                    ...
                    printMedIntegradas.write("Fechas" + "|" + "Hora" +"|"+ "IdMedidor" + "|" + "PotenciaReal" +
                          "|"+"PotenciaReactiva"+"|"+ "NombreArchivo" + "|"+ "MultiploUnidadPotencia" + sbCrLf());
                }
                //------------------------------------------------------------
                ...
                for (int i = 0; i < absArchivo.archHora.size(); i++) {// inicializa el lazo para barrer totos los registros leidos
                    ...
                    //--------------------
                    double archPotenciaReactiva=(double) absArchivo.archPotenciaReactiva.get(i);

                    //---------------
                    String estadoRegistroMedicion= (String) absArchivo.archflagAnormalidad.get(i); // carga el estado de anormalidad de la medicion
                   // double funcModuloFp= (double) absArchivo.archFP.get(i);
                    ...
                    if (varFuncionModulo==1 && funcModuloArchPotencia<0){// Se convierten las potencias negativas a positiva
                     ...
                     //   if(funcModuloFp<0){
                     //       funcModuloFp=(-1) * funcModuloFp;// invertir signo
                     //   }
//
                        estadoRegistroMedicion=" ";//cambia el valor de anormalidad del registro al invertir el signo
                    ...
                    printMedIntegradas.write(absArchivo.archFecha.get(i) + "|" + absArchivo.archHora.get(i) +
                            "|" +absArchivo.archIdMedidorAMI.get(i)+"|"+ funcModuloArchPotencia+"|"+archPotenciaReactiva+
                                 //absArchivo.buscarUsuarioxNombArchivoInstalacion(absArchivo.extraeNombreArchivoBaseSinrutaNiextension(nombreArchivo))+
                                 //  "|"+absArchivo.buscarTarifaUsuarioxNombArchivoInstalacion(absArchivo.extraeNombreArchivoBaseSinrutaNiextension(nombreArchivo))+
                            "|"+ absArchivo.extraeNombreArchivoBaseSinrutaNiextension(nombreArchivo)+//
                ...
                System.out.println("No habilitada la compilacion");// BtnHabIntegrador esta en la clase Setup
                if(varHabilitaCompilador==1) {
                    printErrorArchivoReport.write("Archivo No compilado | " + nombreArchivo + " | " + calificacionArchivoMedicion +
                            " | "+ absArchivo.extraeNombreArchivoBaseSinrutaNiextension(nombreArchivo)  +
                            " |" + absArchivo.buscarTarifaUsuarioxNombArchivoInstalacion(absArchivo.extraeNombreArchivoBaseSinrutaNiextension(nombreArchivo))+
                            " |" +absArchivo.multiploUnidadPotencia+
                            sbCrLf());
                }
            }
        }//-----------------------------------------------------------------------------------------------------------------------
        ...
        listaReporteDetalle.add(sbCrLf()+"xCONFIGURACION: "+ setup.stringConfiguracion()+sbCrLf());
        //---------------------------------------------------------------------------------------------
