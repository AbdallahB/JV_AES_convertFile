...
public class AbsMedicionesFormato_04 extends AbsMediciones {
    ...
    public AbsMedicionesFormato_04(String tipodeFormatoaLeer, JFrame framePrincipal) {
    ...
    public  void PrincipalDevMediciones() throws IOException, ParseException { // Aplicacion principal para el chequeo de las mediciones
        ...
        String palabraFormato4L1="FECHA|HORA|KWH|KVARH|ESTADO";
        ...
        AbsArchivoFormato_04 absArchivo=new AbsArchivoFormato_04(palabraFormato4L1,0,getDirectorio(),tipodeFormatoaLeer,longitudDigitosNombreArchivo) ;// Define el formato a ser leido -->Instancia la clase Archivo e inicializa numerolinea con cero>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>><

        System.out.println(" Formato_04 ------------------------>>>> el directorio definido que va a a clase Achivo es "+directorio);
...
//***********************************************
    ...
    public void escribeResultadosenBufferReporte(AbsArchivoFormato_04 absArchivo) throws IOException {
    ...
    public void lazoprincipal2(AbsArchivoFormato_04 absArchivo, String nombreArchivo,int numlineas) throws IOException, ParseException {
        ...
        for (int l=0;l<xlistaArchivos.size();l++) {
            ...
            if ((setup.CnroAplicacionEspecial == 0) && (tipodeFormatoaLeer.equals("FORMATO_04"))) { // no hay caso especial y se habilita el metodo segun FORMATO XX
                System.out.println("<< ESTA ES LA DERIVACION FORMATO 04 ------------------");
               TokenErrorFormat = absArchivo.generarTokensFormato_04(path, nombreArchivo, numlineas);
            ...
            try {
                String calificacionArchivoMedicion=absArchivo.calificarMedicionM_Formato04_AES_PD(nombreArchivo,   //   !!!!!!!!  LLama al metodo de calificacion de
            ...
            // Factor de Carga
           // Integer tempFactorCarga= Math.toIntExact(Math.round(redondeaEnergiaPromedio / redondeaEnergiaMax * 100));
            ...
            //-----------------------------
           ////Convertir formato de fecha de MM/DD/AAAA a DD/MM/AAAA para utilizar el metodo "CalcularDiasString"
           //String F1=absArchivo.archFecha.get((absArchivo.archFecha.size() - 1)).toString();//Fecha Final
           //String F2=absArchivo.archFecha.get(0).toString();// Fecha Inicial
           //String [] partfechatemporal=F1.split("/");
           //String fechaFinal=partfechatemporal[1]+"/"+partfechatemporal[0]+"/"+partfechatemporal[2];
           //partfechatemporal=F2.split("/");
           //String fechaInicial=partfechatemporal[1]+"/"+partfechatemporal[0]+"/"+partfechatemporal[2];
            ...
            if (flagPresentardiasenResumen(calificacionArchivoMedicion)){// si existe "D" el flag es "False"
            ...
            listaReporteDetalle.add(" Energía reactiva total:        " + tab()+ redondeaEnergiaTotalReactiva + " "+ absArchivo.multiploUnidadPotenciaReactiva+"h"+sbCrLf());
            ...
            for (int calif=0;calif<setup.calificacionesPermitidas.size();calif++){ // Barre todas las claificaciones permitidas en SETUP
                if (setup.calificacionesPermitidas.get(calif).equals(calificacionArchivoMedicion)){
                    System.out.println("calificacion coincidente "+setup.calificacionesPermitidas.get(calif)+"|"+calificacionArchivoMedicion);
