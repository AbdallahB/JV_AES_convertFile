...
import java.util.ArrayList;
    ...
    public class AbsMedicionesFormato_08 extends AbsMediciones {
        public JFrame framePrincipal;

        public AbsMedicionesFormato_08(String tipodeFormatoaLeer) {
        ...
        public AbsMedicionesFormato_08(String tipodeFormatoaLeer, JFrame framePrincipal) {
            super(tipodeFormatoaLeer);
            this.framePrincipal = framePrincipal;
        }
        ...
        public  void PrincipalDevMediciones() throws IOException, ParseException { // Aplicacion principal para el chequeo de las mediciones
            ...
            AbsArchivoFormato_08 absArchivo=new AbsArchivoFormato_08(palabraFormato8L1,0,getDirectorio(),tipodeFormatoaLeer,longitudDigitosNombreArchivo) ;// Define el formato a ser leido -->Instancia la clase Archivo e inicializa numerolinea con cero>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>><

            System.out.println(" Formato_08 ------------------------>>>> el directorio definido que va a a clase Achivo es "+directorio);
...
//***********************************************
        ...
        public void escribeResultadosenBufferReporte(AbsArchivoFormato_08 absArchivo) throws IOException {
        ...
        public void lazoprincipal2(AbsArchivoFormato_08 absArchivo, String nombreArchivo,int numlineas) throws IOException, ParseException {
            ...
            for (int l=0;l<xlistaArchivos.size();l++) {
                ...
                if ((setup.CnroAplicacionEspecial == 0) && (tipodeFormatoaLeer.equals("FORMATO_08"))) { // no hay caso especial y se habilita el metodo segun FORMATO XX
                    ...
                    if ((absArchivo.tipoArchivo == "M")) {             // ******* Define procesamiento de los archivos tipo MONOFASICO
                        try{
                            absArchivo.generarTokensFormato_08Monofasico(path, nombreArchivo, numlineas); //**** llama PROCESAR EL ARCHIVO MONOFASICO CON LAS MUESTRAS
                        } catch (IOException e) {
                            e.printStackTrace();
                            System.out.println("error Formato 8 IO");
                        } catch (ParseException e) {
                            e.printStackTrace();
                            System.out.println("error Formato 8 parse");
                        }
                    } else { // ***************                 Define tratamiento de los archivos TRIFASICOS
                        absArchivo.generarTokensFormato_08Trifasico(path, nombreArchivo, numlineas);
                    }
                }
                if ((setup.CnroAplicacionEspecial == 0) && (tipodeFormatoaLeer.equals("FORMATO_08"))) { // no hay caso especial y se habilita el metodo segun FORMATO XX
                ...
                if(varHabilitaCompilador==1 && suicheCalificacionCompilable==1 &&
                    ...
                    for (int i = 0; i < absArchivo.archHora.size(); i++) {// inicializa el lazo para barrer totos los registros leidos
                        ...
                        printMedIntegradas.write(absArchivo.archFecha.get(i) + "|" + absArchivo.archHora.get(i) + "|" +
                                absArchivo.archVoltaje.get(i) + "|" + funcModuloArchPotencia + "|" + funcModuloFp + "|" +
                                estadoRegistroMedicion + "|"+
                                absArchivo.buscarUsuarioxNombArchivoInstalacion(absArchivo.extraeNombreArchivoBaseSinrutaNiextension(nombreArchivo))+
                                "|"+absArchivo.buscarTarifaUsuarioxNombArchivoInstalacion(absArchivo.extraeNombreArchivoBaseSinrutaNiextension(nombreArchivo))+
                                "|"+ absArchivo.extraeNombreArchivoBaseSinrutaNiextension(nombreArchivo)+//
